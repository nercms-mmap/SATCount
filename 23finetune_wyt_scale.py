...
def get_args_parser():
    ...
    # Dataset parameters
    # parser.add_argument('--data_path', default='/home/cliang/wyt/Bilinear-Matching-Network-main/data/', type=str,
    #                     help='dataset path')

    parser.add_argument('--output_dir', default='./output_dir_wyt23',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./output_dir_wyt',
                        help='path where to tensorboard log')
    parser.add_argument('--device', default='cuda',
                        help='device to use for training / testing')
    parser.add_argument('--seed', default=0, type=int)
    parser.add_argument('--resume', default='./output_fim6_dir/FSC147.pth',
                       help='resume from checkpoint')
    # parser.add_argument('--resume', default='./output_fim6_dir/mae_pretrain_vit_base.pth',
    #                     help='resume from checkpoint')
    # parser.add_argument('--resume', default='./output_dir_wyt/4checkpoint-299.pth',
    #                     help='resume from checkpoint')
...
class TrainData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return sample['image'], sample['gt_density'], sample['boxes'], sample['hw_mean'], sample['m_flag']
...
def main(args):
    ...
    #     n1 = name.split('.')[0]
    #     if n1 in ['patch_embed', 'blocks', 'norm', 'resnet']:
    #         param.requires_grad = False
    # frozen weights 1
    # for name, param in model_without_ddp.named_parameters():
    #     if name.startswith("resnet") or name.startswith("blocks") or name.startswith("patch_embed"):
    #         param.requires_grad = False
    ...
    # misc.load_model_FSC_wyt(args=args, model_without_ddp=model_without_ddp)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, (samples, gt_density, boxes, hw_mean, m_flag) in enumerate(
            ...
            hw_mean = hw_mean.to(device, non_blocking=True).half()
            ...
            with torch.cuda.amp.autocast():
                output = model(samples, boxes, shot_num, hw_mean)
        ...
        if args.output_dir and train_mae / (len(data_loader_train) * args.batch_size) < min_MAE:
            ...
            print('save the best model, epoch：', epoch)
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
        ...
        if args.output_dir and misc.is_main_process():
            # if log_writer is not None:
            #     log_writer.flush()
            with open(os.path.join(args.output_dir, "23lognn1.txt"), mode="a", encoding="utf-8") as f:
