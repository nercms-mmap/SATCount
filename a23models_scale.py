...
from FFN_module import FeatureFusionLayer_scale15
...
class SupervisedMAE(nn.Module):
    def __init__(self, img_size=384, patch_size=16, in_chans=3,
        ...
        self.FFN_scale = FeatureFusionLayer_scale15(d_model=512)
        ...
        self.pos_embedT = nn.Parameter(torch.zeros(1, num_patches, embed_dim),
                                      requires_grad=False)
    ...
    def forward_decoder(self, x, y_, hw_mean, shot_num=3):
        ...
        y = y.transpose(1, 2)  # y [3,N,C]->[N,3,C]

        x = x.permute(1, 0, 2)
        y = y.permute(1, 0, 2)
        # apply FFN
        # x = self.FFN(y, x)
        x = self.FFN_scale(y, x, hw_mean)
        x = x.squeeze(0)
