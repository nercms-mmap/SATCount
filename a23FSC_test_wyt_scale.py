...
# import a23models_wyt_scale_vis as models_mae_cross
# import amodels_wo_hw as models_mae_cross
# import a23models_wyt_scale as models_mae_cross
import a23models_wyt_scaleoldmicro as models_mae_cross
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./Imagewyt',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./Imagewyt',
                        help='path where to tensorboard log')
    parser.add_argument('--device', default='cuda',
                        help='device to use for training / testing')
    parser.add_argument('--seed', default=0, type=int)
    parser.add_argument('--resume', default='/mnt/disk5new/qcj/code/SATCount2/output_fim6_dir/23checkpoint33-666.pth',
...
data_path = '/mnt/disk5new/qcj/code/SATCount2/data/'
...
class TestData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        hw_mean = [hsum / 3.0, wsum / 3.0]
        hw_mean = torch.Tensor(hw_mean)
        ...
        for box in rects:
            ...
            # y1, x1, y2, x2 = box2[0], int(box2[1] * scale_factor), box2[2], int(box2[3] * scale_factor) #test default
            y1, x1, y2, x2 = box2[0], int(box2[1]), box2[2], int(box2[3])   #test 1
        ...
        sample = {'id': im_id,'image': image, 'dots': dots, 'boxes': boxes, 'pos': rects, 'gt_map': gt_map}
        return sample['id'], sample['image'], sample['dots'], sample['boxes'], sample['pos'], sample['gt_map'], hw_mean

def build_activation_fn():
    def _sigmoid(x):
        return 1 / (1 + np.exp(-x))
    return _sigmoid
...
def main(args):
    ...
    misc.load_model_FSC_wyt_test(args=args, model_without_ddp=model_without_ddp)
    # misc.load_model_FSC_wyt_mae(args=args, model_without_ddp=model_without_ddp)
    ...
    for data_iter_step, (id, samples, gt_dots, boxes, pos, gt_map, hw_mean) in enumerate(
        ...
        hw_mean = hw_mean.to(device, non_blocking=True)
        pos = pos
        id = id
        ...
        if s_cnt >= 1:
            ...
            for r_image in r_images:
                ...
                with torch.no_grad():
                    while start + 383 < w:
                        output, = model(r_image[:, :, :, start:start + 384], boxes, 3, hw_mean)
            ...
            with torch.no_grad():
                while start + 383 < w:
                    output, = model(samples[:, :, :, start:start + 384], boxes, 3, hw_mean)
                    output = output.squeeze(0)
                    b1 = nn.ZeroPad2d(padding=(start, w - prev - 1, 0, 0))
                    d1 = b1(output[:, 0:prev - start + 1])
                    b2 = nn.ZeroPad2d(padding=(prev + 1, w - start - 384, 0, 0))
        ...
        #     density_map = activation_fn(density_map)
        #     density_map = (density_map - density_map.min()) / (density_map.max() - density_map.min())
        #
        #     #
        #     s = samples.cpu().detach().numpy()
        #     W, H = s.shape[3], s.shape[2]
        #     new_H = 16 * int(H / 16)
        #     new_W = 16 * int(W / 16)
        #     image_path = os.path.join('/home/cliang/wyt/Bilinear-Matching-Network-main/data/images_384_VarV2', id[0])
        #     image_ = cv2.imread(image_path)
        #     image_ = cv2.resize(image_, (new_W, new_H), interpolation=cv2.INTER_AREA)
        #     image_ = cv2.cvtColor(image_, cv2.COLOR_BGR2RGB)
        #     output_ = apply_scoremap(image_, density_map)
        #     output_ = cv2.cvtColor(output_, cv2.COLOR_RGB2BGR)
        #
        #     filename = id[0].split('.')[0]
        #     resname = "{}_gt{}_pred{}.jpg".format(filename, gt_cnt, pred_cnt)
        #     filepath = os.path.join('./test_image_good10', resname)
        #     cv2.imwrite(filepath, output_)
