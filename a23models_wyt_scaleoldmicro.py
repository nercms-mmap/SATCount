...
# from FFN import FeatureFusionNetwork
# from FFNwyt import FeatureFusionLayerwyt
from FFNwyt import FeatureFusionLayerwyt_scale15  #SATCount
# from FFNwyt import FeatureFusionLayerwyt_scaleNN2
...
class SupervisedMAE(nn.Module):
    def __init__(self, img_size=384, patch_size=16, in_chans=3,
        ...
        # --------------------------------------------------------------------------
        self.resnet = resnet50(output_layers=['layer3'], pretrained=True, frozen_layers=())
        self.input_proj = nn.Conv2d(1024, 512, kernel_size=1)
        self.decoder_embed1 = nn.Linear(64, 512, bias=True)
        # self.FFN = FeatureFusionNetwork(d_model=512, num_featurefusion_layers=3)
        # self.FFNwyt = FeatureFusionLayerwyt(d_model=512)
        self.FFNwyt_scale = FeatureFusionLayerwyt_scale15(d_model=512)
    ...
    def forward_encoder(self, x):
        ...
        return x  # [N, 3, 576, 768]

    def forward_decoder(self, x, y_, hw_mean, shot_num=3):
        # embed tokens
        x = self.decoder_embed(x)  # [N, 3, 576, 512]
        # add pos embed
        ...
        for yi in y_:
            ...
            if cnt > shot_num:
                break
            yi = self.resnet(yi)  #B*1024*8*8
            yi = self.input_proj(yi['layer3']) #B*512*8*8
            yi = yi.flatten(2) #B*512*64
            # yi = self.decoder_embed1(yi) #B*512*512

            N, C, C1 = yi.shape
            y1.append(yi)  # yi [N,C,1,1]->[N,C]

        y = torch.cat(y1, dim=0).reshape(shot_num,N,C,C1)
        y = torch.mean(y, 0).to(x.device)
        y = y.transpose(1, 2)  # y [3,N,C]->[N,3,C]  #B*64*512
        ...
        # x = self.FFN(y, x)
        # x = self.FFNwyt(y, x)
        x = self.FFNwyt_scale(y, x, hw_mean)
        x = x.squeeze(0)
    ...
    def forward(self, imgs, boxes, shot_num, hw_mean):
        with torch.no_grad():
            latent = self.forward_encoder(imgs)
        pred = self.forward_decoder(latent, boxes, hw_mean, shot_num)  # [N, 384, 384]
        return pred
